...
// const fetchLikes = async (postId) => {
//   const response = await axios.get(`http://192.168.123.26:8080/api/getLikes/${postId}`);
...
const Post = forwardRef(
  ({ postId, profile, username, timestamp, description, fileType, fileData, userId,like,userEmail}, ref) => {
    ...
    const deleteBoardMutation = useMutation(
      async () => {
        await axios.delete(`http://192.168.123.26:8080/api/deleteBoard/${postId}`)
      },
      {
        onSuccess: () => {
       ...
       // // 좋아요 상태 가져오기
       useEffect(() => {
        axios.get(`http://192.168.123.26:8080/api/getLiked/${postId}/${email}`)
          .then(response => {
    ...
    const updateLikes = useMutation(
      async ({ postId, newLikesCount, email }) => {
        setLiked(!liked);
        // 좋아요 갯수 업데이트
        await axios.post(`http://192.168.123.26:8080/api/updateLike/${postId}/${newLikesCount}`);
        // 좋아요 삭제하기
        if (liked) {
          await axios.delete(`http://192.168.123.26:8080/api/deleteLike/${postId}/${email}`);
        } else {
          // 좋아요 생성하기
          await axios.post("http://192.168.123.26:8080/api/insertLike", {
            email: email,
            boardid: postId,
          });
        }
